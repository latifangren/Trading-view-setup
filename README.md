...
Untuk pemakaian normal confluence, mulai dari v3 atau RECOMMENDED. Kalau ingin fokus khusus ke session sweep dan reclaim, pakai Session Bias v1. Kalau ingin coba guard lower timeframe dan mode performa yang lebih aman, bandingkan v3 dengan v4. Kalau ingin decision layer v5 tapi alert lebih aman untuk realtime, pakai v6 stable-alert. Kalau ingin alur eksekusi yang sama tapi tampilan default lebih bersih dan dashboard lebih fokus ke keputusan akhir, pakai v7 clean-execution. Di v6 dan v7, sinyal mentah intrabar akan tampil sebagai `PENDING LONG` atau `PENDING SHORT` sampai candle close; alert Confirmed Long, Confirmed Short, dan Invalidated baru aktif setelah close saat `Confirm signals on bar close` menyala. Early Warning tetap setup warning awal, bukan alert close-confirm.
